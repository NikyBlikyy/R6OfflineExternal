...
void SetupWindow()
{
	...
	windowClass.lpszClassName = "R6External";
	RegisterClassExA(&windowClass);

	hWnd = CreateWindowExA(WS_EX_TRANSPARENT | WS_EX_TOPMOST, windowClass.lpszClassName, "R6External", WS_POPUP | WS_VISIBLE, rect.left, rect.top, Width, Height, NULL, NULL, windowClass.hInstance, NULL);
...
void DrawString(const ImVec2& aPos, const std::string& aString, D3DCOLOR color)
{
	auto vList = ImGui::GetBackgroundDrawList();
	vList->AddText(aPos, color, aString.data());
}
